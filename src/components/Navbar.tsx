      ...
      <div className="container mx-auto px-4 sm:px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            />
            <span 
              className="font-bold text-xl sm:text-2xl text-lime-400 dark:text-lime-400"
              onClick={() => navigate('/')}
              style={{ cursor: 'pointer' }}
            >
              CureXpert
          ...
          <nav className="hidden md:flex items-center space-x-6">
            ...
            <div className="relative">
              <NavigationMenu>
                <NavigationMenuList>
                  <NavigationMenuItem>
                    <NavigationMenuTrigger className="bg-transparent hover:bg-lime-900 hover:text-lime-400 text-lime-300">Services</NavigationMenuTrigger>
                    <NavigationMenuContent className="navigation-dropdown">
                      <div className="p-6 bg-lime-950 border border-lime-800 rounded-md shadow-xl w-[650px]">
                        <div className="mb-6 pb-4 border-b border-lime-800">
                          <h3 className="text-lime-400 font-semibold mb-3">Premium Services</h3>
                          <ul className="grid grid-cols-1 gap-3">
                            {services.filter(s => s.isPremium).map((service) => (
                              <li key={service.id}>
                                <button
                                  onClick={() => handleServiceClick(service.id)}
                                  className="block select-none p-2 rounded-md leading-none no-underline outline-none transition-colors hover:bg-lime-900 text-white hover:text-lime-400 w-full text-left"
                                >
                                  {service.name}
                                </button>
                              </li>
                        ...
                        <div>
                          <h3 className="text-lime-400 font-semibold mb-3">General Services</h3>
            ...
            <div className="relative">
              <NavigationMenu>
                <NavigationMenuList>
                  <NavigationMenuItem>
                    <NavigationMenuTrigger className="bg-transparent hover:bg-lime-900 hover:text-lime-400 text-lime-300">Specialties</NavigationMenuTrigger>
                    <NavigationMenuContent className="navigation-dropdown">
                      <ul className="grid grid-cols-3 gap-3 p-6 w-[550px] bg-lime-950 border border-lime-800 rounded-md shadow-xl">
                        {specialties.map((specialty) => (
                          <li key={specialty.id}>
                            <button
                              onClick={() => handleSpecialtyClick(specialty.id)}
                              className="block select-none p-3 rounded-md leading-none no-underline outline-none transition-colors hover:bg-lime-900 text-white hover:text-lime-400 w-full text-left"
                            >
            ...
            <ThemeToggle />
            <Button className="bg-lime-500 hover:bg-lime-600 text-lime-950 font-semibold">
              Book Appointment
            </Button>
          </nav>
        ...
        {isMenuOpen && (
          <div className="md:hidden mt-4 py-4 border-t border-lime-800">
            <nav className="flex flex-col space-y-4">
              {/* Services dropdown for mobile */}
              <div>
                <button
                  onClick={toggleServices}
                  className="flex items-center justify-between w-full text-left text-lime-300 hover:text-lime-400 transition-colors"
                >
                  <span>Services</span>
                  {isServicesOpen ? (
                ...
                {isServicesOpen && (
                  <div className="pl-4 mt-2 space-y-2">
                    <div className="mb-2">
                      <h4 className="text-lime-400 font-semibold mb-1 text-sm">Premium Services</h4>
                      {services.filter(s => s.isPremium).map((service) => (
                        <button
                          key={service.id}
                          className="block text-white hover:text-lime-400 transition-colors py-1 w-full text-left"
                          onClick={() => handleServiceClick(service.id)}
                        >
                          {service.name}
                        </button>
                    ...
                    <div>
                      <h4 className="text-lime-400 font-semibold mb-1 text-sm">General Services</h4>
                      {services.filter(s => !s.isPremium).map((service) => (
                        <button
                          key={service.id}
                          className="block text-white hover:text-lime-400 transition-colors py-1 w-full text-left"
